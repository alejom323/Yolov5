...
# Intentar importar OpenAI sin romper la app si no está instalado
try:
...
#from tensorflow import ...  # comentar si no se usa
...
# Initialize the OpenAI client with the API key (solo si el paquete está disponible)
    ...
    with st.spinner("Analizando ..."):
        ...
        try:
            if openai is None or client is None:
                st.error("El módulo 'openai' no está disponible o la API key no está configurada. Instala 'openai' e ingresa la clave.")
            else:
                full_response = ""
...
```# filepath: /workspaces/drawrecog/app.py
import os
import streamlit as st
import base64
